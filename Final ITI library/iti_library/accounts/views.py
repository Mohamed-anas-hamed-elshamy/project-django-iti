...
class EditProfileView(LoginRequiredMixin, UpdateView):
    model = CustomUser
    form_class = CustomUserChangeForm
    template_name = 'accounts/updataprofile.html'
    success_url = reverse_lazy('accounts.profile')
...
def admin_user_list(request):
    ...
    return render(request, 'accounts/allusers.html', {'page_obj': page_obj, 'search_query': search_query})


@user_passes_test(is_admin)
def admin_user_profile(request, user_id):
    user = get_object_or_404(User, id=user_id)
    return render(request, 'accounts/dashboard.html', {'user': user, 'is_admin_view': True})